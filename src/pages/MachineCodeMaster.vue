      ...
      columns: [
        ...
        },
        {
          ...
          formatter(cell) {
            const response: Response = cell.getData();
            return `<div>
                <div class="font-medium whitespace-nowrap">${response.machinename}</div>
              </div>`;
          },
        },
        {
  ...
  axios.get(`http://172.188.122.62:8085/api/MachineCodeMaster/${id}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        editFormData.machinename = firstItem.machinename || '';
    ...
    if (result.isConfirmed) {
      ...
      axios.post('http://172.188.122.62:8085/api/MachineCodeMaster', {
        ...
        .then(response => {
          ...
          addFormData.machinecode = '';
          addFormData.machinename = '';
...
<template>
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Machine Code Master</h2>
    <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
      <Button variant="primary" class="mr-2 shadow-md" as="a" href="#" @click="handleAddClick">
        Add Machine Code Master
      </Button>
    </div>
  </div>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="addRole">
          <div>
            ...
            <FormInput id="regular-form-1" v-model="addFormData.machinecode" type="text" placeholder="Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Name</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.machinename" type="text" placeholder="Name" />
          </div>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="updateRole">
          <div>
            ...
            <FormInput id="edit-form-code" v-model="editFormData.machinecode" type="text" placeholder="Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="edit-form-name">Name</FormLabel>
            <FormInput id="edit-form-name" v-model="editFormData.machinename" type="text" placeholder="Name" />
          </div>
