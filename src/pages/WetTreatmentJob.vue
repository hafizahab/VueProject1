        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess && response.status !== "No Jobs") {
                    // Conditionally render "Edit" link based on wetTreatmentJob permissions
                    if (userAccess.wetTreatmentJob && userAccess.wetTreatmentJob.includes("Edit")) {
                    ...
                    // Conditionally render "Delete" link based on wetTreatmentJob permissions
                    if (userAccess.wetTreatmentJob && userAccess.wetTreatmentJob.includes("Delete")) {
...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/WetTreatment/GetWetTreatmentJobs';
...
// Define permission variables for WetTreatmentJob
let canEditWetTreatmentJob = false;
const canCreateWetTreatmentJob = ref(false);
...
const fetchUserWetTreatmentPermissions = async () => {
  ...
  try {
    ...
    if (userDetails) {
      ...
      const wetTreatmentJob = userDetails.wetTreatmentJob || ''; // Ensure the value is a string

      // Check if the string contains "Create", "Delete", and "Edit" for WetTreatmentJob
      canCreateWetTreatmentJob.value = wetTreatmentJob.includes('Create');
...
const fetchAndSetData = (machineCode: string, formData: any, modalSetter: (value: boolean) => void, isView: boolean = false) => {
  ...
  axios.get(`http://10.87.0.33:8082/api/WetTreatment/GetWTDetailsbyMachineCode/${machineCode}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        const initializedJobs = filteredData.map((job: any) => ({
          ...job,
          bins: job.listBinforWT || [], // Include listBinforWT directly
        }));
...
const refreshData = () => {
  ...
  // Also run fetchBinOptions to refresh bin options, passing editFormData as an argument
  fetchBinOptions(editFormData);
};
...
const updateJobStatus = async (jobId: string, subJobId: string, machineId: string) => {
  ...
  try {
    ...
    const jobStatusResponse = await axios.post('http://10.87.0.33:8082/api/WetTreatment/UpdateWetTreatmentJobStatus', payload);
    if (jobStatusResponse.status === 200) {
      ...
      for (const bin of existingBins) {
        const updatedBin = {
          ...
          movement: 'Wet Treatment', // Set weight to 0
        };
      ...
      addBinsToJob(job, job.bins); // Post new bins with actual weights
  ...
  try {
    const uniqueBins = job.bins.reduce((unique: any[], bin: any) => {
      const isDuplicate = unique.some(
        uBin => uBin.binMasterId === bin.binMasterId && uBin.mappingId === bin.mappingId
      );
      if (!isDuplicate) {
        unique.push(bin);
      }
...
<template>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Wet Treatment Job</h2>
