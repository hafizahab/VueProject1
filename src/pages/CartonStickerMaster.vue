...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/CartonStickerMaster/';
...
const API_BASE_URL_DELETE = 'http://10.87.0.33:8082/api/CartonStickerMaster/GetCartonStickerMasterDeletedList/';
const API_BASE_URL_UPDATE = 'http://10.87.0.33:8082/api/CartonStickerMaster/UpdateCartonStickerMasterActive';
onMounted(() => {
    // Fetch data from API
...
const addRole = () => {
  ...
  if (formSubmitted.value && (!addFormData.name || !addFormData.status)) {
    ...
    axios.post(API_BASE_URL, { ...addFormData })
      .then(response => {
        if (response.data.message === `Unable to complete request. Carton Sticker data is with Name ${addFormData.name} already exist`) {
          Swal.fire({
            icon: 'error',
            title: 'Duplicate Name',
...
<template>
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Carton Sticker Master</h2>
    <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
      <Button variant="primary" class="mr-2 shadow-md" as="a" href="#" @click="handleAddClick">
        Add Carton Sticker Master
      </Button>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Carton Sticker Master - Deleted List</h2>
    ...
    <Slideover.Panel>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="addRole">
          ...
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Template</FormLabel><span class="text-red-500 pl-1 text-md">*</span>
            <FormSelect :class="{ 'border-red-500': formSubmitted && !addFormData.template }" v-model="addFormData.template"
              class=" sm:mr-2" aria-label="Default select example">
              <option disabled value="">Select a Template Option</option>
              <option>Template1</option>
              <option>Template2</option>
              <option>Template3</option>
              <option>Template4</option>
            </FormSelect>
            <span v-if="formSubmitted && addFormData.template === ''" class="text-red-500">Please choose a
              Template!</span>
          </div>
          <div class="mt-5">
    ...
    <Slideover.Panel>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="updateRole">
          ...
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Template</FormLabel>
            <span class="text-red-500 pl-1 text-md">*</span>
            <FormSelect :class="{ 'border-red-500': !editFormData.template }" v-model="editFormData.template"
              class="sm:mr-2" aria-label="Default select example">
              <option disabled value="">Select a Template Option</option>
              <option>Template1</option>
              <option>Template2</option>
            ...
            <span v-if="editFormData.template === ''" class="text-red-500">Please choose a Template!</span>
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Remarks</FormLabel>
