        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess) {
                    ...
                    if (userAccess.roleMaster && userAccess.roleMaster.includes("Edit")) {
                    ...
                    if (userAccess.roleMaster && userAccess.roleMaster.includes("Delete")) {
...
const API_BASE_URL_DELETE = 'http://10.87.0.33:8082/api/RoleMaster/GetRoleMasterDeletedList/';
const API_BASE_URL_UPDATE = 'http://10.87.0.33:8082/api/RoleMaster/UpdateRoleMasterActive';
onMounted(() => {
    // Fetch data from API
...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/RoleMaster/';

onMounted(() => {
    loading.value = true; // Show loading animation at the start

    // Define a promise for data fetching
    const fetchPromise = axios.get(API_BASE_URL)
        .then(response => {
            const filteredData = response.data.result.filter((item: any) => item.status !== 'InActive');

            // Sort data alphabetically by code
    ...
    // Initialize Tabulator and resize listeners
    initTabulator();
    reInitOnResizeWindow();
});
    ...
    axios.get(`${API_BASE_URL}${id}`)
        .then(response => {
            ...
            if (data && data.length > 0) {
                ...
                modalSetter(true); // Open modal after data is set
            } else {
...
const updateRole = () => {
    ...
    const existingUser = items.value.find(
        user => user.name === editFormData.name || user.code === editFormData.code && user.id !== editFormData.id
    );
    ...
    if (existingUser) {
        Swal.fire({
            ...
            html: `Role Master Data already exists`,
...
const addRole = () => {
    ...
    if (formSubmitted.value && (!addFormData.name || !addFormData.status)) {
        ...
        axios.post(API_BASE_URL, { ...addFormData })
            .then(response => {
                if (response.data.message === `Unable to complete request. Role Master Data is already exist`) {
                    Swal.fire({
                        icon: 'error',
...
const canCreateRoleMaster = ref(false);
...
const fetchUserVendorPermissions = async () => {
    ...
    try {
        ...
        if (userDetails) {
            const roleMaster = userDetails.roleMaster || '';

            // Update permissions reactively
            canCreateRoleMaster.value = roleMaster.includes('Create');
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
...
<template>
  ...
  <div v-if="loading" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    ...
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Role Master</h2>
        <div class="flex w-full mt-4 sm:w-auto sm:mt-0" v-if=canCreateRoleMaster>
    ...
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Role Master - Deleted List</h2>
