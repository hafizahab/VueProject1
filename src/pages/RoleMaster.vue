...
onMounted(() => {
  // Fetch data from API
  axios.get('http://172.188.122.62:8085/api/RoleMaster')
    .then(response => {
...
const editRole = (id: string) => {
  ...
  axios.get(`http://172.188.122.62:8085/api/RoleMaster/${id}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        editFormData.code = firstItem.code || '';
        editFormData.name = firstItem.name || '';
...
const updateRole = () => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      if (editFormData.id) {
        axios.put(`http://172.188.122.62:8085/api/RoleMaster/${editFormData.id}`, {
...
const addRole = () => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      axios.post('http://172.188.122.62:8085/api/RoleMaster', {
...
const deleteRole = (id: string) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      axios.delete(`http://172.188.122.62:8085/api/RoleMaster/${id}`)
        .then(response => {
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="addRole">
          <div>
            ...
            <FormInput id="regular-form-1" v-model="addFormData.code" type="text" placeholder="Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Name</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.name" type="text" placeholder="Name" />
          </div>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="updateRole">
          <div>
            ...
            <FormInput id="edit-form-code" v-model="editFormData.code" type="text" placeholder="Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="edit-form-name">Name</FormLabel>
            <FormInput id="edit-form-name" v-model="editFormData.name" type="text" placeholder="Name" />
          </div>
