...
const onId = () => {
  if (tabulator.value) {
    tabulator.value.setFilter("id", 'like', filter.id);
  }
};
...
const updateRole = (status: 'draft' | 'submitted') => {
  // Show a confirmation dialog before making the API call
  Swal.fire({
    title: 'Confirmation',
    text: `Are you sure you want to ${status === 'draft' ? 'save' : 'submit'} this GR?`,
    icon: 'warning',
    ...
    if (result.isConfirmed && editFormData.id) {
      ...
      axios.put(`http://172.188.122.62:8085/api/GRDetails/${editFormData.id}`, {
        ...
        .then(response => {
          ...
          Swal.fire({
            icon: 'success',
            title: `GR ${status === 'draft' ? 'Saved' : 'Submitted'} Successfully`,
    ...
    <div class="flex flex-col mb-5 sm:flex-row sm:items-end xl:items-start">
      <form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto" @submit="(e) => {
        ...
        <div class="items-center mt-2 sm:flex sm:mr-4 xl:mt-0">
          ...
          <FormInput id="tabulator-html-filter-id" v-model="filter.id" type="text"
        ...
        <Button id="tabulator-html-filter-go" variant="primary" type="button"
          class="w-full ml-10 mt-2 mr-2 sm: ml-0 sm:w-16 sm:mt-0 sm:ml-1" @click="onId">
          ID
        </Button>
        <div class="items-center mt-2 sm:flex sm:mr-4 xl:mt-0">
          <label class="flex-none w-12 mr-2 xl:w-auto xl:flex-initial">
      ...
      <Slideover.Description>
        <form class="validate-form">
          ...
          <div>
            ...
            <FormInput id="regular-form-1" v-model="addFormData.grnumber" type="text" placeholder="GR Number"
              disabled />
          </div>
          <div class="mt-5">
          ...
          <div class="mt-5">
            ...
            <FormInput id="regular-form-1" v-model="addFormData.ponumber" type="text" placeholder="PO Number" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Lot Number</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.lotnumber" type="text" placeholder="Lot Number" />
          </div>
          <div class="mt-5">
          ...
          <div class="mt-5">
            ...
            <FormSelect class="mt-2" id="warehouse-dropdown" v-model="addFormData.warehouselocation">
              <option v-for="warehouseCode in addFormData.warehouseCodes" :key="warehouseCode" :value="warehouseCode">
                {{ warehouseCode }}
              </option>
            </FormSelect>
          </div>
          <Button variant="primary" class="w-[100%] mt-10 shadow-md" @click.prevent="() => addRole('submitted')">
            Submit
          </Button>
    ...
    <Slideover.Panel>
      ...
      <Slideover.Description>
        <form class="validate-form">
          <div>
            <FormLabel htmlFor="regular-form-1">GR Number</FormLabel>
            <FormInput id="regular-form-1" v-model="editFormData.grnumber" type="text" placeholder="GR Number" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Powder Code</FormLabel>
          ...
          <div class="mt-5">
            <label for="vendor-dropdown">Vendor</label>
            <FormSelect class="mt-2" id="vendor-dropdown" v-model="editFormData.vendorcode">
              <option v-for="vendorCode in editFormData.vendorCodes" :key="vendorCode" :value="vendorCode">
                {{ vendorCode }}
              </option>
            </FormSelect>
          </div>
          ...
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Gross</FormLabel>
            <FormInput id="regular-form-1" v-model="editFormData.gross" type="text" placeholder="Gross" />
          </div>
          <div class="mt-5">
            <label for="vendor-dropdown">UOM</label>
          ...
          <div class="mt-5">
            <label for="warehouse-dropdown">Warehouse</label>
            <FormSelect class="mt-2" id="warehouse-dropdown" v-model="editFormData.warehouselocation">
              <option v-for="warehouseCode in editFormData.warehouseCodes" :key="warehouseCode" :value="warehouseCode">
                {{ warehouseCode }}
              </option>
            </FormSelect>
          </div>
          <Button variant="primary" class="w-[100%] mt-10 shadow-md" @click.prevent="() => updateRole('submitted')">
            Submit
          </Button>

          <Button variant="outline-primary" class="w-[100%] mr-2 mt-2 shadow-md"
            @click.prevent="() => updateRole('draft')">
            Save
          </Button>
        </form>
      </Slideover.Description>
...
//   "grnumber": "GR-93350",
//   "powdercode": "pcode02",
//   "vendorcode": "VendorCode01",
//   "ponumber": "POnumber02",
//   "lotnumber": "lotnumber02",
//   "gross": 4,
//   "uom": "KG",
//   "actualweight": 0,
//   "receivedweight": 70.2,
//   "warehouselocation": "MATERIALWH",
//   "status": "WHApprove",
