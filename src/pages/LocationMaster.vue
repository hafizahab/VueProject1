    ...
    formatter(cell) {
        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess) {
                    // Conditionally render "Edit" link if access includes 'Edit'
                    if (userAccess.locationMaster && userAccess.locationMaster.includes("Edit")) {
                    ...
                    // Conditionally render "Delete" link if access includes 'Delete'
                    if (userAccess.locationMaster && userAccess.locationMaster.includes("Delete")) {
...
const API_BASE_URL_DELETE = 'http://10.87.0.33:8082/api/LocationMaster/GetLocationMasterDeletedList/';
const API_BASE_URL_UPDATE = 'http://10.87.0.33:8082/api/LocationMaster/UpdateLocationMasterActive';
onMounted(() => {
    // Fetch data from API
...
const loading = ref<boolean>(false); // Loading state to control animation
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/LocationMaster/';
...
onMounted(() => {
    ...
    axios.get(API_BASE_URL)
        .then(response => {
            ...
            // Custom sorting by name alphabetically
        ...
        .finally(() => {
            loading.value = false; // Hide loading animation once data is set
        });

    // Initialize other functions that don’t need to await loading
    initTabulator();
    reInitOnResizeWindow();
    fetchItems();
});
...
// Define reactive permission variables for locationMaster
const canEditLocationMaster = ref(false);
const canDeleteLocationMaster = ref(false);
const canCreateLocationMaster = ref(false);

// Define the fetchUserLocationPermissions function
const fetchUserLocationPermissions = async () => {
    ...
    try {
        ...
        if (userDetails) {
            ...
            const locationMaster = userDetails.locationMaster || ''; // Ensure the value is a string

            // Check if the string contains "Create", "Delete", and "Edit" for locationMaster
            canCreateLocationMaster.value = locationMaster.includes('Create');
            canDeleteLocationMaster.value = locationMaster.includes('Delete');
            canEditLocationMaster.value = locationMaster.includes('Edit');

            console.log('canCreateLocationMaster:', canCreateLocationMaster.value); // Debugging log
            console.log('canDeleteLocationMaster:', canDeleteLocationMaster.value); // Debugging log
            console.log('canEditLocationMaster:', canEditLocationMaster.value); // Debugging log
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
...
onMounted(async () => {
    await fetchUserLocationPermissions();
});


const addRow = (formData: any) => {
    formData.subLocList.push({
        name: '',
    });
};
...
const DELETE_API_BASE_URL = 'http://10.87.0.33:8082/api/LocationMaster/DeleteSubLocationById/';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${DELETE_API_BASE_URL}${id}`)
          .then(() => {
            editFormData.subLocList.splice(index, 1);
            Swal.fire({
              icon: 'success',
...
const deleteAddRow = (index: number) => {
    addFormData.subLocList.splice(index, 1);
};
...
const fetchAndSetData = (id: string, formData: any, modalSetter: (value: boolean) => void, isView: boolean = false) => {
    console.log(`Fetching data for ID: ${id}`); // Log for debugging
    loading.value = true; // Show loading animation
        ...
        .then(response => {
            ...
            if (data && data.length > 0) {
                ...
                Object.assign(formData, {
                    ...
                    isSubLocation: firstItem.isSubLocation,
                    subLocList: firstItem.subLocationDetailsList || []
                });

                modalSetter(true); // Open modal after data is ready
                console.log('subLocList:', formData.subLocList);
            } else {
...
const addRole = () => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            console.log('Payload for Add Role:', JSON.stringify(payload, null, 2)); // Console log the payload
...
<template>
  ...
  <div v-if="loading" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    ...
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Location Master</h2>
        <div class="flex w-full mt-4 sm:w-auto sm:mt-0" v-if='canCreateLocationMaster'>
    ...
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Location Master - Deleted List</h2>
        ...
        <Slideover.Panel>
            ...
            <Slideover.Description>
                <form class="validate-form" @submit.prevent="addRole">
                    <div>
                        <FormLabel htmlFor="regular-form-1">Main Location</FormLabel><span
                    ...
                    <div class="mt-5">
                        <FormCheck>
                            ...
                            <FormCheck.Input id="pre-blend" v-model="addFormData.isSubLocation" type="checkbox"
                                value="" />
                        </FormCheck>
                    </div>
                    <div v-if="addFormData.isSubLocation">
                        <h6 class="mb-2 mt-8 text-lg font-medium leading-none">Sub Location</h6>
                        <hr>
                        <button type="button" @click="addRowToAddForm"
                            class="mt-5 px-4 py-2 bg-blue-700 text-white rounded hover:bg-blue-900">Add</button>

                        <div v-if="addFormData.subLocList.length > 0">
                            <table class="min-w-full border-collapse rounded-lg overflow-hidden shadow-sm mt-2">
                                <thead class="bg-primary text-white">
                                    <tr>
                                        <th class="py-2 px-4 border-b text-left" style="width: 50px;">No.</th>
                                        <!-- Set the width here -->
                                ...
                                <tbody>
                                    <tr v-for="(row, index) in addFormData.subLocList" :key="index"
                                        class="hover:bg-gray-100">
                                        <td class="py-2 px-4 border-b">
                                            {{ index + 1 }}
                                        </td>
                                        <td class="py-2 px-2 border-b">
                                            <FormInput id="regular-form-1" v-model="row.name" type="text"
                                                placeholder="Name" />
                                        </td>
        ...
        <Slideover.Panel>
            ...
            <Slideover.Description>
                <form class="validate-form" @submit.prevent="updateRole">
                    <div>
                        <FormLabel htmlFor="regular-form-1">Main Location</FormLabel>
                    ...
                    <div class="mt-5">
                        <FormCheck>
                            ...
                            <FormCheck.Input id="pre-blend" v-model="editFormData.isSubLocation" type="checkbox"
                                value="" />
                        </FormCheck>
                    </div>
                    <div v-if="editFormData.isSubLocation">
                        <h6 class="mb-2 mt-8 text-lg font-medium leading-none">Sub Location</h6>
                        <hr>
                        <button type="button" @click="addRowToEditForm"
                            class="mt-5 px-4 py-2 bg-blue-700 text-white rounded hover:bg-blue-900">Add</button>

                        <div v-if="editFormData.subLocList.length > 0">
                            <table class="min-w-full border-collapse rounded-lg overflow-hidden shadow-sm mt-2">
                                <thead class="bg-primary text-white">
                                    <tr>
                                        <th class="py-2 px-4 border-b text-left" style="width: 50px;">No.</th>
                                ...
                                <tbody>
                                    <tr v-for="(row, index) in editFormData.subLocList" :key="index"
                                        class="hover:bg-gray-100">
                                        <td class="py-2 px-4 border-b">
                                            {{ index + 1 }}
                                        </td>
...
:staticBackdrop="true" size="xl" :open="viewModal" @close="() => {
        ...
        <Dialog.Panel>
            ...
            <Dialog.Description class="max-h-[70vh] overflow-y-auto">
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <tbody class="w-[100%]">
                                ...
                                <tr class="border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td class="w-[50%] px-auto text-center">
                                        <span :class="{
                                            'text-green-600 dark:text-green-500': viewData.isSubLocation === true,
                                            'text-red-600 dark:text-red-500': viewData.isSubLocation === false,
                                        }">
                                            {{ viewData.isSubLocation === true ? 'Required' : viewData.isSubLocation === false ? 'Not Required' : 'N/A' }}
                                        </span>
                                    </td>
                                </tr>
                            </tbody>
                ...
                <div v-if="viewData.subLocList.length > 0" class="relative overflow-x-auto shadow-md sm:rounded-lg mt-10">
  <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    <tbody class="w-[100%]">
      <tr class="border-b border-gray-200 dark:border-gray-700">
      ...
      <tr v-for="(row, index) in viewData.subLocList" :key="index" class="hover:bg-gray-100">
        <td class="py-3 text-center text-gray-700 dark:text-white">
          {{ index + 1 }}
        </td>
