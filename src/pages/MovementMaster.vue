...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/MovementMaster/';
...
const addRole = () => {
  ...
  if (formSubmitted.value && (!addFormData.name || !addFormData.status)) {
    ...
    axios.post(API_BASE_URL, { ...addFormData })
      .then(response => {
        if (response.data.message === `Unable to complete request. Movement data is with Name ${addFormData.name} already exist`) {
          Swal.fire({
            icon: 'error',
            title: 'Duplicate Name',
...
<template>
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Movement Master</h2>
