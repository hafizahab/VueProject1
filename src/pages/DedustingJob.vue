    ...
    formatter(cell) {
        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess) {
                    // Conditionally render "Edit" link based on dedustingJob permissions
                    if (
                        response.status !== "No Jobs" &&
                        userAccess.dedustingJob &&
                        userAccess.dedustingJob.includes("Edit")
                    ) {
                    ...
                    // Conditionally render "View" link (always for valid jobs)
                    ...
                    // Conditionally render "Delete" link based on dedustingJob permissions
                    if (
                        response.status !== "No Jobs" &&
                        userAccess.dedustingJob &&
                        userAccess.dedustingJob.includes("Delete")
                    ) {
                        ...
                        deleteLink.addEventListener("click", () => {
                            deleteRole(machineCode); // Pass machineId
                        });
                        deleteLink.innerHTML = `<i class="fa-regular fa-trash-can mr-1"></i>Delete`;
                        container.appendChild(deleteLink);
                    }
                } else {
                    // If user access not found
                    container.innerHTML = `<span class="text-red-500 text-sm">No Access</span>`;
                }
...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/Dedusting/GetDedustingJobs';
...
onMounted(() => {
  ...
  const fetchPromises = [
    ...
    // Additional asynchronous data-fetching functions
  ...
  Promise.all(fetchPromises)
    ...
    .finally(() => {
      loading.value = false; // Hide loading modal once all tasks are complete
    });
...
// Define permission variables for dedustingJob
let canEditDedustingJob = false;

// Define the fetchUserDedustingJobPermissions function
const fetchUserDedustingJobPermissions = async () => {
  ...
  try {
    ...
    if (userDetails) {
      ...
      const dedustingJob = userDetails.dedustingJob || ''; // Ensure the value is a string

      // Check if the string contains "Edit" for dedustingJob
      canEditDedustingJob = dedustingJob.includes('Edit');

      console.log('canEditDedustingJob:', canEditDedustingJob); // Debugging log
    } else {
      Swal.fire({
        icon: 'error',
        title: 'Error',
...
// Call the fetchUserDedustingJobPermissions function to get permissions
fetchUserDedustingJobPermissions();
...
const fetchAndSetData = (machineCode: string, formData: any, modalSetter: (value: boolean) => void, isView: boolean = false) => {
  ...
  axios.get(`http://10.87.0.33:8082/api/Dedusting/GetDedustingDetailsbyMachineCode/${machineCode}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        const initializedJobs = filteredData.map((job: any) => ({
          ...job,
          bins: job.listBinforDD || [], // Include listBinforDD directly
        }));
        ...
        axios.all([
          ...
          // Set job bin names using helper function
          setJobBinNameFromListBins(formData.jobs, listBins, binMaster);

          // Fetch bin options for dropdowns
          ...
          loading.value = false; // Hide loading animation after fetching all data
        });
      } else {
...
const updateJobStatus = async (jobId: string, subJobId: string, machineId: string) => {
  ...
  try {
    ...
    const jobStatusResponse = await axios.post('http://10.87.0.33:8082/api/Dedusting/UpdateDedustingJobStatus', payload);
    if (jobStatusResponse.status === 200) {
      ...
      for (const bin of existingBins) {
        const updatedBin = {
          ...
          movement: 'Dedusting', // Set movement to Dedusting
        };
...
const addBinsToJob = (job: any, bins: any[]) => {
  if (bins) {
    bins.forEach((bin, index) => {
      ...
      axios.post('http://10.87.0.33:8082/api/ListBins/AddListBinsfromDedustingJobs', binPayload)
        .then(response => {
...
<template>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Dedusting Job</h2>
...
:staticBackdrop="true" size="2xl" :open="editModal" @close="() => {
  ...
  <td colspan="9">
    <h3 class="text-lg font-bold mt-4 mb-2 flex items-center dark:text-gray-300">
      <span class="pr-10">List Bin</span>
      <span :class="remainingWeight(jobIndex) === 0 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'"
        class="text-sm font-normal">
        Total Weight: {{ totalBinWeight(jobIndex) }}
      </span>
      <span :class="remainingWeight(jobIndex) === 0 ? 'pl-3 text-green-500 dark:text-green-400' : 'pl-3 text-red-500 dark:text-red-400'"
        class="text-sm font-normal">
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border dark:border-gray-700">
      <thead class="bg-gray-200 dark:bg-gray-800">
        <tr>
          ...
          <th class="px-4 py-2 dark:border-gray-700">Bin Name</th>
          <th class="px-4 py-2 medium-width dark:border-gray-700">Weight</th>
          <th class="px-4 py-2 hidden dark:border-gray-700">Type</th>
          <th class="px-4 py-2 hidden dark:border-gray-700">Colour</th>
          <th class="px-4 py-2 hidden dark:border-gray-700">Capacity Max</th>
          <th class="px-4 py-2 text-center dark:border-gray-700">Delete</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(bin, binIndex) in job.bins" :key="binIndex"
          ...
          <td class="px-4 py-2 text-center dark:text-gray-300">
            <FormCheck>
              <FormCheck.Input type="checkbox" :value="binIndex"
                ...
                @change="toggleBinSelection(jobIndex, binIndex)"
                class="mx-auto dark:bg-gray-800 dark:border-gray-600" />
            </FormCheck>
          </td>
          <td class="px-4 py-2 dark:border-gray-700 dark:text-gray-300">
            <div class="relative">
              <div
                @click="!isBinSubmitted(jobIndex, binIndex) && toggleBinDropdown(jobIndex, binIndex)"
                :class="['border rounded p-2 cursor-pointer dark:bg-gray-800 dark:text-gray-300', { 'cursor-not-allowed opacity-50': isBinSubmitted(jobIndex, binIndex) }]">
                {{ bin.binMasterId === job.binMasterId ? job.binName : getBinName(bin.binMasterId) || 'Select a Bin Name' }}
              </div>
              ...
              <div
                ...
                <input type="text" v-model="binSearchQueryMap[`${jobIndex}-${binIndex}`]" placeholder="Search Bin Name"
          ...
          <td class="px-4 py-2 dark:border-gray-700">
            <div class="flex items-center space-x-2">
              <FormInput type="number" v-model="bin.weight" class="w-full p-2 rounded border-gray-300 dark:bg-gray-800 dark:text-gray-300"
                placeholder="Weight" :disabled="isBinSubmitted(jobIndex, binIndex)" />
              <button type="button" @click="syncWeight(jobIndex, binIndex)"
                class="bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-500 my-2 mx-2 px-2 py-2 text-white rounded">
                <i class="fa fa-refresh"></i>
              </button>
            </div>
          </td>
          <td class="px-4 py-2 hidden dark:border-gray-700 dark:text-gray-300">{{ bin.type }}</td>
          <td class="px-4 py-2 hidden dark:border-gray-700 dark:text-gray-300">{{ bin.colour }}</td>
          <td class="px-4 py-2 hidden dark:border-gray-700 dark:text-gray-300">{{ bin.capacityMax }}</td>
          <td class="px-2 py-2 small-width text-center dark:border-gray-700">
            <button @click="deleteBinField(jobIndex, binIndex)" :disabled="job.status === 'Completed'"
              :class="{ 'text-gray-400': job.status === 'Completed', 'text-red-600 hover:text-red-800 dark:hover:text-red-400': job.status !== 'Completed' }">
