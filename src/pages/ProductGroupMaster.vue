...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/ProductGroupMaster/';
...
const addRole = () => {
    ...
    axios.post(API_BASE_URL, { ...addFormData })
      .then(response => {
        if (response.data.message === `Unable to complete request. Shift data is with Name ${addFormData.name} already exist`) {
          Swal.fire({
            icon: 'error',
            title: 'Duplicate Name',
