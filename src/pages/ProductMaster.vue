      ...
      columns: [
        ...
        },
        {
          ...
          formatter(cell) {
            const response: Response = cell.getData();
            return `<div>
                <div class="font-medium whitespace-nowrap">${response.powdername}</div>
              </div>`;
          },
        },
        {
          ...
          formatter(cell) {
            const response: Response = cell.getData();
            return `<div>
                <div class="font-medium whitespace-nowrap">${response.receiptpercentage} %</div>
              </div>`;
          },
        },
...
onMounted(() => {
  // Fetch data from API
  axios.get('http://172.188.122.62:8085/api/ProductMaster')
    .then(response => {
...
const editRole = (id: string) => {
  ...
  axios.get(`http://172.188.122.62:8085/api/ProductMaster/${id}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        editFormData.productcode = firstItem.productcode || '';
        editFormData.powdercode = firstItem.powdercode || '';
        editFormData.powdername = firstItem.powdername || '';
        editFormData.receiptpercentage = firstItem.receiptpercentage || '';
...
const updateRole = () => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      if (editFormData.id) {
        axios.put(`http://172.188.122.62:8085/api/ProductMaster/${editFormData.id}`, {
          productcode: editFormData.productcode,
          powdercode: editFormData.powdercode,
          powdername: editFormData.powdername,
          receiptpercentage: editFormData.receiptpercentage,
...
const addRole = () => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      axios.post('http://172.188.122.62:8085/api/ProductMaster', {
        productcode: addFormData.productcode,
        powdercode: addFormData.powdercode,
        powdername: addFormData.powdername,
        receiptpercentage: addFormData.receiptpercentage,
        ...
        .then(response => {
          ...
          addFormData.powdername = '';
          addFormData.receiptpercentage = '';
...
const deleteRole = (id: string) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      axios.delete(`http://172.188.122.62:8085/api/ProductMaster/${id}`)
        .then(response => {
...
<template>
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Product Master</h2>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="addRole">
          <div>
            ...
            <FormInput id="regular-form-1" v-model="addFormData.productcode" type="text" placeholder="Name" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Powder Code</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.powdercode" type="text" placeholder="Powder Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Powder Name</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.powdername" type="text" placeholder="Powder Name" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Receipt Percentage (%)</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.receiptpercentage" type="text" inputmode="numeric"
              pattern="[0-9]*[.,]?[0-9]+" placeholder="Receipt Percentage (%)" min="1" max="100" />
          </div>
          <!-- <div class="mt-5">
    ...
    <Slideover.Panel>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="updateRole">
          <div>
            <FormLabel htmlFor="regular-form-1">Product Code</FormLabel>
            <FormInput id="regular-form-1" v-model="editFormData.productcode" type="text" placeholder="Product Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Powder Code</FormLabel>
            <FormInput id="regular-form-1" v-model="editFormData.powdercode" type="text" placeholder="Powder Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Powder Name</FormLabel>
            <FormInput id="regular-form-1" v-model="editFormData.powdername" type="text" placeholder="Powder Name" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Receipt Percentage (%)</FormLabel>
            <FormInput id="regular-form-1" v-model="editFormData.receiptpercentage" type="text" inputmode="numeric"
              pattern="[0-9]*[.,]?[0-9]+" placeholder="Receipt Percentage (%)" min="1" max="100" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Status</FormLabel>
            <FormSelect v-model="editFormData.status" class=" sm:mr-2" aria-label="Default select example">
              <option>active</option>
              <option>inactive</option>
