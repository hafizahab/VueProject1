            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter(cell) {
                        const response: Response = cell.getData();
                        return `<div>
                <div class="font-medium whitespace-nowrap">${response.location}</div>
              </div>`;
                    },
                },
                {
...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/MachineLocationMaster/';
    ...
    axios.get('http://10.87.0.33:8082/api/MachineMaster')
        .then(response => {
            ...
            if (Array.isArray(nameData)) {
                ...
                addFormData.machineCodes = activeNameData.map(item => item.name);
...
// Update Role
const updateRole = () => {
    if (!editFormData.machineCode || !editFormData.location || !editFormData.status) {
    ...
    const existingUser = items.value.find(
        user => user.machineCode === editFormData.machineCode && user.id !== editFormData.id
    );

    if (existingUser) {
        Swal.fire({
            icon: 'error',
            title: 'Duplicate Machine Code',
            html: `<b>${editFormData.machineCode}</b> already exists, please try another name`,
...
const addRole = () => {
    ...
    if (formSubmitted.value && (!addFormData.machineCode || !addFormData.location || !addFormData.status)) {
        // If form is submitted but jobId is empty, show an alert to complete the form
        Swal.fire({
            icon: 'error',
            title: 'Please complete the field',
        ...
        axios.post(API_BASE_URL, { ...addFormData })
            .then(response => {
                if (response.data.message === `Unable to complete request. Machine Location Master Data is already exist for Machine Code - ${addFormData.machineCode}`) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Duplicate Name',
                        html: `<b>${addFormData.machineCode}</b> already exists, please try another name`,
                        confirmButtonText: 'Try Again',
                        confirmButtonColor: '#d33',
                    });
                } else {
...
<template>
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Machine Location Master</h2>
        <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
            <Button variant="primary" class="mr-2 shadow-md" as="a" href="#" @click="handleAddClick">
                Add Machine Location Master
            </Button>
        ...
        <Slideover.Panel>
            ...
            <Slideover.Description>
                <form class="validate-form" @submit.prevent="addRole">
                    ...
                    <div>
      <FormLabel htmlFor="regular-form-5">Machine Code</FormLabel>
      <FormSelect v-model="addFormData.machineCode"
          :class="{ 'border-red-500': formSubmitted && !addFormData.machineCode }"
          class=" sm:mr-2" aria-label="Default select example">
        <option disabled value="">Select a Machine Code Option</option>
        <option v-for="name in addFormData.machineCodes" :key="name" :value="name">
          {{ name }}
        </option>
      </FormSelect>
      <span v-if="formSubmitted && !addFormData.machineCode" class="text-red-500">Machine Code is required!</span>
    </div>
                    <div class="mt-5">
      <FormLabel htmlFor="regular-form-5">Location</FormLabel>
      <FormSelect v-model="addFormData.location"
          :class="{ 'border-red-500': formSubmitted && !addFormData.location }"
          class=" sm:mr-2" aria-label="Default select example">
        <option disabled value="">Select a Location Option</option>
        <option v-for="name in addFormData.locations" :key="name" :value="name">
          {{ name }}
        </option>
      </FormSelect>
        ...
        <Slideover.Panel>
            ...
            <Slideover.Description>
                <form class="validate-form" @submit.prevent="updateRole">
                    <div>
      <FormLabel htmlFor="regular-form-5">Machine Code</FormLabel>
      <FormSelect v-model="editFormData.machineCode"
          :class="{ 'border-red-500': !editFormData.machineCode }"
          class=" sm:mr-2" aria-label="Default select example">
        <option disabled value="">Select a Machine Code Option</option>
        <option v-for="name in editFormData.machineCodes" :key="name" :value="name">
          {{ name }}
        </option>
      </FormSelect>
      <span v-if="!editFormData.machineCode" class="text-red-500">Machine Code is required!</span>
    </div>
    <div class="mt-5">
      <FormLabel htmlFor="regular-form-5">Location</FormLabel>
      <FormSelect v-model="editFormData.location"
          :class="{ 'border-red-500': !editFormData.location }"
          class=" sm:mr-2" aria-label="Default select example">
        <option disabled value="">Select a Location Option</option>
        <option v-for="name in editFormData.locations" :key="name" :value="name">
          {{ name }}
        </option>
      </FormSelect>
