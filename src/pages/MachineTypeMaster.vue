        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess) {
                    ...
                    if (userAccess.machineTypeMaster && userAccess.machineTypeMaster.includes("Edit")) {
                    ...
                    if (userAccess.machineTypeMaster && userAccess.machineTypeMaster.includes("Delete")) {
...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/MachineTypeMaster/';
...
const API_BASE_URL_DELETE = 'http://10.87.0.33:8082/api/MachineTypeMaster/GetMachineTypeMasterDeletedList/';
const API_BASE_URL_UPDATE = 'http://10.87.0.33:8082/api/MachineTypeMaster/UpdateMachineTypeMasterActive';
onMounted(() => {
    // Fetch data from API
...
// Define reactive permission variables for machineTypeMaster
const canEditMachineTypeMaster = ref(false);
const canDeleteMachineTypeMaster = ref(false);
const canCreateMachineTypeMaster = ref(false);

// Define the fetchUserMachineTypePermissions function
const fetchUserMachineTypePermissions = async () => {
    ...
    try {
        ...
        if (userDetails) {
            ...
            const machineTypeMaster = userDetails.machineTypeMaster || ''; // Ensure the value is a string

            // Check if the string contains "Create", "Delete", and "Edit" for machineTypeMaster
            canCreateMachineTypeMaster.value = machineTypeMaster.includes('Create');
            canDeleteMachineTypeMaster.value = machineTypeMaster.includes('Delete');
            canEditMachineTypeMaster.value = machineTypeMaster.includes('Edit');

            console.log('canCreateMachineTypeMaster:', canCreateMachineTypeMaster.value); // Debugging log
            console.log('canDeleteMachineTypeMaster:', canDeleteMachineTypeMaster.value); // Debugging log
            console.log('canEditMachineTypeMaster:', canEditMachineTypeMaster.value); // Debugging log
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
...
onMounted(async () => {
    await fetchUserMachineTypePermissions();
});
...
<template>
  ...
  <div v-if="loading" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    ...
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Machine Type Master</h2>
        <div class="flex w-full mt-4 sm:w-auto sm:mt-0" v-if="canCreateMachineTypeMaster">
            <Button variant="primary" class="mr-2 shadow-md" as="a" href="#" @click="handleAddClick">
                Add Machine Type Master
            </Button>
    ...
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Machine Type Master - Deleted List</h2>
