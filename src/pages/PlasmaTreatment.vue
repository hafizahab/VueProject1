        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess) {
                    ...
                    if (
                        userAccess.plasmaTreatmentJob &&
                        userAccess.plasmaTreatmentJob.includes("Edit") &&
                ...
                // Always render the "View" link if machine has jobs
...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/PlasmaTreatment/GetPlasmaTreatmentJobs';
...
// Define permission variables for plasmaTreatmentJob
let canEditPlasmaTreatmentJob = false;
const canCreatePlasmaTreatmentJob = ref(false);

// Define the fetchUserPlasmaTreatmentPermissions function
const fetchUserPlasmaTreatmentPermissions = async () => {
  ...
  try {
    ...
    if (userDetails) {
      ...
      const plasmaTreatmentJob = userDetails.plasmaTreatmentJob || ''; // Ensure the value is a string

      // Check if the string contains "Create", "Delete", and "Edit" for plasmaTreatmentJob
      canCreatePlasmaTreatmentJob.value = plasmaTreatmentJob.includes('Create');
      canEditPlasmaTreatmentJob = plasmaTreatmentJob.includes('Edit');

      console.log('canEditPlasmaTreatmentJob:', canEditPlasmaTreatmentJob); // Debugging log
    } else {
      Swal.fire({
        icon: 'error',
        title: 'Error',
...
// Call the fetchUserPlasmaTreatmentPermissions function to get permissions
fetchUserPlasmaTreatmentPermissions();
...
const addBinField = (jobIndex: number) => {
  ...
  editFormData.jobs[jobIndex].bins.push({
    binMasterId: '',
    weight: 0,
    type: 'Plasma Treatment',
    colour: '',
    capacityMax: 0,
  });

  // Now update bins based on the jobId
...
const fetchAndSetData = (machineCode: string, formData: any, modalSetter: (value: boolean) => void, isView: boolean = false) => {
  ...
  axios.get(`http://10.87.0.33:8082/api/PlasmaTreatment/GetPTDetailsbyMachineCode/${machineCode}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        const initializedJobs = filteredData.map((job: any) => ({
          ...job,
          bins: job.listBinforPT || [], // Include listBinforPT directly
        }));
...
const updateJobStatus = async (jobId: string, subJobId: string, machineId: string) => {
  ...
  console.log('Job Status Payload:', JSON.stringify(payload, null, 2)); // Log the payload before status update
  ...
  try {
    ...
    for (const bin of job.bins) {
      const binPayload = {
        ...
        type: bin.type || 'Plasma Treatment',
        weight: bin.weight || 0,
      };
    ...
    const jobStatusResponse = await axios.post('http://10.87.0.33:8082/api/PlasmaTreatment/UpdatePlasmaTreatmentJobStatus', payload);
    if (jobStatusResponse.status === 200) {
      ...
      // Step 5: Post new bins with actual weights entered in the UI
      addBinsToJob(job, job.bins);

      // Step 6: Refresh the list if needed
...
const addBinsToJob = (job: any, bins: any[]) => {
  if (bins) {
    bins.forEach((bin, index) => {
      ...
      console.log(`Submitting payload for bin ${index + 1}:`, JSON.stringify(binPayload, null, 2));

      axios.post('http://10.87.0.33:8082/api/ListBins/AddListBinsfromPlasmaTreatmentJobs', binPayload)
        .then(response => {
          console.log(`Bin ${index + 1} added:`, response.data);
          fetchListBins();
        })
        .catch(error => {
          console.error(`Error adding bin ${index + 1}:`, error);
        });
    });
  }
};
...
<template>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Plasma Treatment Job</h2>
