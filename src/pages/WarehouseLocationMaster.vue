      ...
      columns: [
        ...
        },
        {
          ...
          formatter(cell) {
            ...
            return `<div>
                <div class="font-medium whitespace-nowrap">${response.warehouselocationcode}</div>
              </div>`;
          },
        },
        {
          ...
          formatter(cell) {
            const response: Response = cell.getData();
            return `<div>
                <div class="font-medium whitespace-nowrap">${response.warehouselocationname}</div>
              </div>`;
          },
        },
        {
  ...
  axios.get(`http://172.188.122.62:8085/api/WarehouseLocation/${id}`)
    .then(response => {
      ...
      if (data && data.length > 0) {
        ...
        editFormData.warehouselocationcode = firstItem.warehouselocationcode || '';
        editFormData.warehouselocationname = firstItem.warehouselocationname || '';
    ...
    if (result.isConfirmed) {
      ...
      axios.post('http://172.188.122.62:8085/api/WarehouseLocation', {
        ...
        .then(response => {
          ...
          addFormData.warehouselocationcode = '';
          addFormData.warehouselocationname = '';
...
<template>
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Warehouse Location Master</h2>
    <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
      <Button variant="primary" class="mr-2 shadow-md" as="a" href="#" @click="handleAddClick">
        Add Warehouse Location Master
      </Button>
    </div>
  </div>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="addRole">
          <div>
            ...
            <FormInput id="regular-form-1" v-model="addFormData.warehouselocationcode" type="text" placeholder="Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="regular-form-1">Name</FormLabel>
            <FormInput id="regular-form-1" v-model="addFormData.warehouselocationname" type="text" placeholder="Name" />
          </div>
          <!-- <div class="mt-5">
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="updateRole">
          <div>
            ...
            <FormInput id="edit-form-code" v-model="editFormData.warehouselocationcode" type="text"
              placeholder="Code" />
          </div>
          <div class="mt-5">
            <FormLabel htmlFor="edit-form-name">Name</FormLabel>
            <FormInput id="edit-form-name" v-model="editFormData.warehouselocationname" type="text"
              placeholder="Name" />
          </div>
