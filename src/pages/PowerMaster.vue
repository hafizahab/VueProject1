        ...
        getUserAccessData()
            .then((userAccessData) => {
                ...
                if (userAccess) {
                    ...
                    if (userAccess.powderMaster && userAccess.powderMaster.includes("Edit")) {
                    ...
                    if (userAccess.powderMaster && userAccess.powderMaster.includes("Delete")) {
...
onMounted(() => {
  ...
  axios.get(API_BASE_URL)
    .then(response => {
      const filteredData = response.data.result.filter((item: any) => item.status !== 'InActive' && item.productType === 'Powder');
...
onMounted(() => {
    ...
    .then(response => {
            ...
            const filteredData = response.data.result.filter((item : any) => item.productType === 'Powder');
...
const editFormData = reactive({
  ...
  chemicaltreatmentrequired: false,
  blowrequired: false,
...
const fetchUserPartNumberPermissions = async () => {
    ...
    try {
        ...
        if (userDetails) {
            ...
            const powderMaster = userDetails.powderMaster || ''; // Ensure the value is a string

            // Check if the string contains "Create", "Delete", and "Edit" for partNumberMaster
            canCreatePartNumberMaster.value = powderMaster.includes('Create');
            canDeletePartNumberMaster.value = powderMaster.includes('Delete');
            canEditPartNumberMaster.value = powderMaster.includes('Edit');
...
watch(() => addFormData.type, (newValue) => {
  if (newValue === 'Mixed Powder') {
    addFormData.containsRecipe = true;
  } else {
...
const fetchCartontypes = () => {
  axios.get('http://10.87.0.33:8082/api/CartonMaster')
    .then(response => {
      ...
      if (Array.isArray(nameData)) {
        ...
        addFormData.cartonTypes = activeNameData.map(item => item.name);
        editFormData.cartonTypes = activeNameData.map(item => item.name);
      } else {
...
// Updated fetchAndSetData function with formatting applied
...
const updateRole = () => {
  ...
  // Set discrepencyWeight to 0 if it's empty, null, or undefined
  if (!editFormData.discrepencyWeight) {
    editFormData.discrepencyWeight = 0;
  }

  // Check if any required fields are missing (including the 'type' and discrepencyWeight validation)
  if (!editFormData.code || !editFormData.status || editFormData.type === '*' ||
      ...
      (editFormData.discrepencyWeight !== null && editFormData.discrepencyWeight !== undefined &&
       (isNaN(editFormData.discrepencyWeight) || // Check if it's a valid number
        editFormData.discrepencyWeight.toString().split('.')[1]?.length > 4))) { // Check decimal length
  ...
  if (editFormData.containsRecipe) {
    const totalPercentage = editFormData.productRecipe.reduce((sum, row) => {
      ...
      return sum + parseFloat(percentage.toFixed(4));
    }, 0);
    ...
    if (Math.abs(totalPercentage - 100) > marginOfError || hasInvalidDecimals) {
      Swal.fire({
        ...
        text: totalPercentage !== 100
          ...
          : 'Percentage values must have at most 4 decimal places!',
...
const addRole = () => {
  ...
  // Set discrepencyWeight to 0 if it's empty, null, or undefined
  if (!addFormData.discrepencyWeight) {
    addFormData.discrepencyWeight = 0;
  }

  // Check if any required fields are missing (code, type, and discrepencyWeight validation)
  if (
    !addFormData.code ||
    !addFormData.status ||
    addFormData.type === '*' ||
    (addFormData.containsRecipe && addFormData.productRecipe.length === 0) ||
    (addFormData.discrepencyWeight !== null && addFormData.discrepencyWeight !== undefined &&
     (isNaN(addFormData.discrepencyWeight) || // Check if it's a valid number
      addFormData.discrepencyWeight.toString().split('.')[1]?.length > 4)) // Directly check decimal length without rounding
  ) {
    Swal.fire({
      icon: 'error',
  ...
  if (addFormData.containsRecipe) {
    ...
    // Check if any percentage has more than 4 decimal places
    const hasInvalidDecimals = addFormData.productRecipe.some(row => {
      ...
      const decimalPart = row.percentage.toString().split('.')[1];
      // Check if decimal part exists and has more than 4 digits
      return decimalPart && decimalPart.length > 4;
    });
  ...
  const formData = {
    ...addFormData,
    platform: Array.isArray(addFormData.platform) ? addFormData.platform.join(',') : addFormData.platform,
  };
...
const resetFormData = (formData: any) => {
  ...
  addFormData.code = '';
  addFormData.productType = 'Powder';
  addFormData.containsRecipe = false;
  addFormData.secondaryProcess = false;
  addFormData.chemicaltreatmentrequired = false;
  addFormData.blowrequired = false;
  ...
  addFormData.type = select2.value;
  addFormData.discrepencyWeight = 0;
  select.value = "*",
...
const resetEditFormData = () => {
  ...
  select.value = "*";
  editFormData.type = select2.value;
  editFormData.discrepencyWeight = 0;
...
<template>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    <h2 class="mr-auto text-lg font-medium">Powder Master</h2>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Powder Master - Deleted List</h2>
  ...
  <TomSelect :class="{ 'border-red-500': formSubmitted && addFormData.type === '*' }" v-model="addFormData.type" :options="{ placeholder: 'Select a Type Option' }" class="w-full">
    <option disabled value="*">Select a Type Option</option>
    <option>Single Powder</option>
    <option>Mixed Powder</option>
    <option>Tip Mix Powder</option>
  </TomSelect>
  <span v-if="formSubmitted && addFormData.type === '*'" class="text-red-500">Type is required!</span>
</div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              <tbody class="w-[100%]">
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700">
                  <th scope="row"
                    class="px-4 py-2 text-center font-medium text-white whitespace-nowrap bg-primary dark:text-white dark:bg-gray-800">
                    Time Taken for Chemical Treatment per KG (HH:MM:SS)
                  </th>
                  <th scope="row"
                    class="px-4 py-2 text-center font-medium text-white whitespace-nowrap bg-primary dark:text-white dark:bg-gray-800">
                    Time Taken for Blow per KG (HH:MM:SS)
                  </th>
                </tr>
                <tr>
                  <td class="px-4 py-2 text-center text-gray-700 dark:text-white">
                  ...
                  <td class="px-4 py-2 text-center text-gray-700 dark:text-white">
                    <div class="time-picker flex items-center justify-center">
                      <FormSelect v-model="addFormData.hoursForBlow" name="hours" class="mr-1">
                        <option v-for="n in 25" :key="n" :value="n - 1">{{ formatNumber(n - 1) }}</option>
                      </FormSelect>
                      :
                      <FormSelect v-model="addFormData.minutesForBlow" name="minutes" class="mx-1">
                        <option v-for="n in 61" :key="n" :value="n - 1">{{ formatNumber(n - 1) }}</option>
                      </FormSelect>
                      :
                      <FormSelect v-model="addFormData.secondsForBlow" name="seconds" class="ml-1">
                        <option v-for="n in 61" :key="n" :value="n - 1">{{ formatNumber(n - 1) }}</option>
                      </FormSelect>
                    </div>
                  </td>
          ...
          <div v-if="addFormData.productType === 'Product'" class="mt-5">
            <FormCheck>
              <FormCheck.Label htmlFor="secondaryProcess" class="mr-2 ml-[-2]">
                Secondary Process
              </FormCheck.Label>
              <FormCheck.Input id="secondaryProcess" v-model="addFormData.secondaryProcess" type="checkbox" value="" />
            </FormCheck>
          </div>
          <div v-if="addFormData.productType === 'Product'" class="mt-5">
            <FormCheck>
              <FormCheck.Label htmlFor="chemical-treatment" class="mr-2 ml-[-2]">
                Chemical Treatment Required
              </FormCheck.Label>
              <FormCheck.Input id="chemical-treatment" v-model="addFormData.chemicaltreatmentrequired" type="checkbox"
                value="" />
            </FormCheck>
          </div>
          ...
          <div v-if="addFormData.productType === 'Product'" class="mt-5">
            <FormCheck>
              <FormCheck.Label htmlFor="blow-required" class="mr-2 ml-[-2]">
                Blow Required
              </FormCheck.Label>
              <FormCheck.Input id="blow-required" v-model="addFormData.blowrequired" type="checkbox" value="" />
            </FormCheck>
          </div>
          ...
          <div v-if="addFormData.productType === 'Product'" class="mt-5">
            <FormLabel htmlFor="weight-per-piece">Weight Per Piece (in grams)</FormLabel>
            <FormInput id="weight-per-piece" v-model="addFormData.weightperpiece" type="text" placeholder="Weight" />
          </div>
          <!-- <div v-if="addFormData.productType === 'Powder'" class="mt-5">
  <FormLabel htmlFor="discrepencyWeight">Discrepency Weight</FormLabel>
  <FormInput
    id="discrepencyWeight"
    v-model="addFormData.discrepencyWeight"
    type="text"
    placeholder="Discrepency Weight"
    :class="{'border-red-500': formSubmitted && (isNaN(addFormData.discrepencyWeight) || addFormData.discrepencyWeight.toString().split('.')[1]?.length > 4)}"
  />
  <span v-if="formSubmitted && isNaN(addFormData.discrepencyWeight)" class="text-red-500">
    Discrepency Weight must be a valid number!
  </span>
  <span v-if="formSubmitted && !isNaN(addFormData.discrepencyWeight) && addFormData.discrepencyWeight.toString().split('.')[1]?.length > 4" class="text-red-500">
    Discrepency Weight must have at most 4 decimal places!
  </span>
</div> -->
          ...
          <div class="mt-5">
            <FormCheck>
              ...
              <FormCheck.Input id="pre-blend" v-model="addFormData.containsRecipe" type="checkbox"
                :disabled="addFormData.type === 'Mixed Powder'" />
            </FormCheck>
          </div>
          <!-- Product Recipe -->
          ...
          <div class="relative">
            ...
            <span v-if="formSubmitted && !row.code" class="text-red-500">Product Code is required!</span>
                    ...
                    <td class="py-2 px-4 border-b small-width">
                      <FormInput
                        :class="{ 'border-red-500': formSubmitted && (!row.percentage || !isNumeric(row.percentage) || row.percentage.toString().split('.')[1]?.length > 4) }"
                        v-model="row.percentage" type="text" class="w-full px-2 py-1 border rounded small-width" />
                      <span
                        v-if="formSubmitted && (!row.percentage || !isNumeric(row.percentage) || row.percentage.toString().split('.')[1]?.length > 4)"
                        class="text-red-500">
                        {{ !row.percentage ? 'Percentage is required!' : !isNumeric(row.percentage) ? 'Percentage must  be a valid number!' : 'Percentage must be a valid number with up to 4 decimal points!' }}
                       </span>
                    </td>
    ...
    <Slideover.Panel>
      ...
      <Slideover.Description>
        <form class="validate-form" @submit.prevent="updateRole">
          ...
          <div class="mt-5">
  <FormLabel htmlFor="regular-form-5">Type</FormLabel><span class="text-red-500 pl-1 text-md">*</span>
  <TomSelect :class="{ 'border-red-500': formSubmitted && editFormData.type === '*' }" v-model="editFormData.type" :options="{ placeholder: 'Select a Type Option' }" class="w-full">
    <option disabled value="*">Select a Type Option</option>
    <option>Single Powder</option> <!-- Corrected -->
    <option>Mixed Powder</option>  <!-- Corrected -->
    <option>Tip Mix Powder</option> <!-- Corrected -->
  </TomSelect>
  <span v-if="formSubmitted && editFormData.type === '*'" class="text-red-500">Type is required!</span>
</div>
          ...
          <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-10"
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              <tbody class="w-[100%]">
                ...
                <!-- Second Row: Chemical Treatment and Blow -->
                <tr class="border-b border-gray-200 dark:border-gray-700">
                  <th scope="row"
                    class="px-4 py-2 text-center font-medium text-white whitespace-nowrap bg-primary dark:text-white dark:bg-gray-800">
                    Time Taken for Chemical Treatment per KG (HH:MM:SS)
                ...
                <tr>
                  ...
                  <td class="px-4 py-2 text-center text-gray-700 dark:text-white">
                    <div class="time-picker flex items-center justify-center">
                      <FormSelect v-model="editFormData.hoursForBlow" name="hours" class="mr-1">
                        <option v-for="n in 25" :key="n" :value="n - 1">{{ formatNumber(n - 1) }}</option>
                      </FormSelect>
                      :
                      <FormSelect v-model="editFormData.minutesForBlow" name="minutes" class="mx-1">
                        <option v-for="n in 61" :key="n" :value="n - 1">{{ formatNumber(n - 1) }}</option>
                      </FormSelect>
                      :
                      <FormSelect v-model="editFormData.secondsForBlow" name="seconds" class="ml-1">
                        <option v-for="n in 61" :key="n" :value="n - 1">{{ formatNumber(n - 1) }}</option>
                      </FormSelect>
                    </div>
                  </td>
          ...
          <div v-if="editFormData.productType === 'Product'" class="mt-5">
            <FormCheck>
              ...
              <FormCheck.Input id="secondaryProcess" v-model="editFormData.secondaryProcess" type="checkbox" value="" />
            </FormCheck>
          </div>
          <div v-if="editFormData.productType === 'Product'" class="mt-5">
            <FormCheck>
              ...
              <FormCheck.Input id="pre-blend" v-model="editFormData.chemicaltreatmentrequired" type="checkbox"
                value="" />
            </FormCheck>
          </div>
          ...
          <div v-if="editFormData.productType === 'Product'" class="mt-5">
            <FormCheck>
              ...
              <FormCheck.Input id="pre-blend" v-model="editFormData.blowrequired" type="checkbox" value="" />
            </FormCheck>
          </div>
          <div v-if="editFormData.productType === 'Powder'" class="mt-5">
            <FormCheck>
              ...
              <FormCheck.Input id="pre-blend" v-model="editFormData.qualitycheckrequired" type="checkbox" value="" />
            </FormCheck>
          </div>
          <div v-if="editFormData.productType === 'Product'" class="mt-5">
            <FormLabel htmlFor="regular-form-4">Weight Per Piece (in grams)</FormLabel>
  ...
  <FormInput
    id="discrepencyWeight"
    v-model="editFormData.discrepencyWeight"
    type="text"
    placeholder="Discrepency Weight"
    :class="{'border-red-500': editFormData.discrepencyWeight !== null && editFormData.discrepencyWeight !== undefined &&
    (isNaN(editFormData.discrepencyWeight) || editFormData.discrepencyWeight.toString().split('.')[1]?.length > 4)}"
  />
  <span v-if="editFormData.discrepencyWeight !== null && editFormData.discrepencyWeight !== undefined && isNaN(editFormData.discrepencyWeight)" class="text-red-500">
    Discrepency Weight must be a valid number!
  </span>
  <span v-if="editFormData.discrepencyWeight !== null && editFormData.discrepencyWeight !== undefined && !isNaN(editFormData.discrepencyWeight) && editFormData.discrepencyWeight.toString().split('.')[1]?.length > 4" class="text-red-500">
    Discrepency Weight must have at most 4 decimal places!
  </span>
</div> -->
          ...
          <div class="mt-5">
            <FormCheck>
              ...
              <FormCheck.Input id="pre-blend" v-model="editFormData.containsRecipe" type="checkbox"
                :disabled="editFormData.type === 'Mixed Powder'" />
            </FormCheck>
          </div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              <tbody class="w-[100%]">
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700">
                  ...
                  <td
                    :class="['w-[70%] px-auto text-center', viewData.qualitycheckrequired ? 'text-green-500' : 'text-red-500']">
  ...
  <td class="w-[70%] px-auto text-center text-gray-700 dark:text-white">
    ...
    <div v-else>
      {{ viewData.platform }}
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400"
              v-if="viewData.productType === 'Product'">
              <tbody class="w-[100%]">
                <tr class="border-b border-gray-200 dark:border-gray-700">
                  <th scope="row"
                    class="w-[30%] px-6 py-4 text-center font-medium text-white whitespace-nowrap bg-primary dark:text-white dark:bg-gray-800  ">
                    Time Taken for <br> Chemical Treatment per KG
                  </th>
                  <td class="w-[70%] px-auto text-center text-gray-700 dark:text-white">
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700" v-if="viewData.productType === 'Product'">
                  <th scope="row"
                    class="w-[30%] px-6 py-4 text-center font-medium text-white whitespace-nowrap bg-primary dark:text-white dark:bg-gray-800  ">
                    Time Taken for <br> Blow per KG
                  </th>
                  <td class="w-[70%] px-auto text-center text-gray-700 dark:text-white">
                    {{ viewData.timeTakenforBlow }}
                  </td>
                </tr>
