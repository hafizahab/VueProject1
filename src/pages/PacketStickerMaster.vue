...
// API Base URL
const API_BASE_URL = 'http://10.87.0.33:8082/api/PacketStickerMaster/';
...
const API_BASE_URL_DELETE = 'http://10.87.0.33:8082/api/PacketStickerMaster/GetPacketStickerMasterDeletedList/';
const API_BASE_URL_UPDATE = 'http://10.87.0.33:8082/api/PacketStickerMaster/UpdatePacketStickerMasterActive';
onMounted(() => {
    // Fetch data from API
...
const addRole = () => {
  ...
  if (formSubmitted.value && (!addFormData.name || !addFormData.status)) {
    ...
    axios.post(API_BASE_URL, { ...addFormData })
      .then(response => {
        if (response.data.message === `Unable to complete request. Packet Sticker data is with Name ${addFormData.name} already exist`) {
          Swal.fire({
            icon: 'error',
            title: 'Duplicate Name',
...
<template>
  ...
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Packet Sticker Master - Deleted List</h2>
